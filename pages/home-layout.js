// pages/home-layout.js - Dynamic Home Layout Configuration with Preview Mode
...
export default function HomeLayoutManager() {
  ...
  const fetchLayout = useCallback(async () => {
    try {
      ...
      // Initialize with empty layout on error
      setLayout({ sections: [] })
    } finally {
      setLoading(false)
    }
    ...
    try {
      ...
      const sampleProducts = [
        ...
        },
        {
          ...
          price: 49.99,
          main_picture_url: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&h=300&fit=crop',
          shipping_price: 0
        }
      ]
...
}// pages/home-layout.js - Dynamic Home Layout Configuration
import { useState, useEffect, useCallback } from 'react'
import { useRouter } from 'next/router'
import { supabase } from '../lib/supabase'
import DashboardLayout from '../components/DashboardLayout'
...
export default function HomeLayoutManager() {
  ...
  const addSection = () => {
    const newSection = {
      ...
      type: 'product_horizontal',
      title: 'New Section',
      position: layout.sections.length + 1,
      config: {
        limit: 10,
        source: 'products',
        filter: {}
